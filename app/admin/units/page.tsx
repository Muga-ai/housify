 ...
 * - ADDED delete button per unit row
 * - Delete clears tenant assignment before removing unit
 * - writeBatch used for atomic updates (already was)
...
export default function AdminUnitsPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    rentAmount: "", rentDueDay: "1", tenantId: "",
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingUnit) {
        batch.update(doc(db, "units", editingUnit.id), {
        ...
        if (prevTenantId && prevTenantId !== newTenantId) {
          batch.update(doc(db, "tenants", prevTenantId), { unitId: null, propertyId: null });
        }
        // Stamp new tenant
      ...
      if (editingUnit) {
        setUnits((prev) =>
          prev.map((u) =>
            u.id === editingUnit.id
              ? { ...u, propertyId: formData.propertyId, unitNumber: formData.unitNumber,
                  rentAmount: Number(formData.rentAmount), rentDueDay: Number(formData.rentDueDay),
                  tenantId: newTenantId, status }
              : u
          )
        );
        setTenants((prev) =>
          prev.map((t) => {
            if (t.id === prevTenantId && prevTenantId !== newTenantId) return { ...t, unitId: null };
            if (t.id === newTenantId) return { ...t, unitId: editingUnit.id };
            return t;
          })
        );
      } else {
        const unitSnap = await getDocs(query(collection(db, "units"), where("orgId", "==", orgId)));
        setUnits(unitSnap.docs.map((d) => ({ id: d.id, ...d.data() } as Unit)));
      }
  ...
  const deleteUnit = async (unit: Unit) => {
    ...
    try {
      ...
      // Clear tenant assignment if occupied
      if (unit.tenantId) {
        batch.update(doc(db, "tenants", unit.tenantId), {
      ...
      if (unit.tenantId) {
        setTenants((prev) =>
          prev.map((t) =>
            t.id === unit.tenantId ? { ...t, unitId: null } : t
          )
        );
      }
    } catch (err) {
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl max-w-xl w-full p-8 space-y-5">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <input required placeholder="Unit Number (e.g. A1, 101)"
              ...
              <div>
                ...
                <input required type="number" min="1" placeholder="e.g. 25000"
              ...
              <div>
                ...
                <select required value={formData.rentDueDay}
              ...
              <div>
                ...
                <p className="text-xs text-gray-400 mt-1">
                  Assigning updates the tenant dashboard with rent details automatically.
                </p>
              </div>
