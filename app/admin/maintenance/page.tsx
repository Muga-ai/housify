 ...
 * - Real-time listener now filters by orgId
 */
...
interface MaintenanceRequest {
  id: string; property: string; unit: string; tenant: string;
...
export default function AdminMaintenancePage() {
  ...
  useEffect(() => {
    ...
    const q = query(
      ...
      where("orgId", "==", orgId),           // ← scoped to org
      ...
      <div className="mx-auto max-w-7xl space-y-6">
        <header className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-600">View and manage all tenant-reported issues</p>
          </div>
          ...
          {filterOptions.map((option) => (
            <button
              ...
              className={`px-4 py-2 rounded-md text-sm font-medium transition ${
                filter === option ? "bg-indigo-600 text-white" : "bg-white border text-gray-700 hover:bg-gray-50"
              }`}
            >
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={7} className="px-6 py-12 text-center text-gray-500">
                  {search || filter !== "all" ? "No matching requests found." : "No maintenance requests yet."}
                </td></tr>
              ) : (
                filteredRequests.map((req, index) => (
                  <tr key={req.id} className="border-t hover:bg-gray-50">
                    ...
                    <td className="px-6 py-3">
                      <button onClick={() => setSelected(req)} className="text-indigo-600 hover:underline text-sm font-medium">
                        View Details
                      </button>
                    </td>
                  </tr>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-xl max-w-2xl w-full max-h-screen overflow-y-auto p-6 space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 text-sm">
              <div><span className="font-medium text-gray-600">Property:</span><p className="mt-1">{selected.property}</p></div>
              <div><span className="font-medium text-gray-600">Unit:</span><p className="mt-1">{selected.unit}</p></div>
              <div><span className="font-medium text-gray-600">Tenant:</span><p className="mt-1">{selected.tenant}</p></div>
              <div><span className="font-medium text-gray-600">Submitted:</span><p className="mt-1">{selected.submitted}</p></div>
            </div>
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => updateStatus(selected.id, e.target.value as MaintenanceRequest["status"])}
            ...
            <div className="flex justify-end">
              <button onClick={() => setSelected(null)} className="rounded-md bg-indigo-600 px-6 py-2 text-white font-medium hover:bg-indigo-700 transition">
                Close
              </button>
            </div>
          </div>
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span className={`inline-block px-3 py-1 rounded-full text-xs font-medium ${map[status] ?? ""}`}>
