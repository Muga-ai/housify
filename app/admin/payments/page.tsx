 ...
 * See all submitted payments, verify or reject with reason.
 */
...
export default function AdminPaymentsPage() {
  ...
  useEffect(() => {
    ...
    const fetchPayments = async () => {
      try {
        ...
        const rawPayments = paymentsSnap.docs.map(
          (d) => ({ id: d.id, ...d.data() } as Payment)
        );

        // Fetch tenants and units to join names
        const [tenantSnap, unitSnap, propertySnap] = await Promise.all([
          ...
          getDocs(
            query(collection(db, "properties"), where("orgId", "==", orgId))
          ),
        ]);

        const tenantMap = Object.fromEntries(
          tenantSnap.docs.map((d) => [d.id, d.data()])
        );
        const unitMap = Object.fromEntries(
          unitSnap.docs.map((d) => [d.id, d.data()])
        );
        const propertyMap = Object.fromEntries(
          propertySnap.docs.map((d) => [d.id, d.data()])
        );
  ...
  const thisMonthVerified = thisMonthPayments.filter(
    (p) => p.status === "verified"
  );
  const thisMonthRevenue = thisMonthVerified.reduce(
    (sum, p) => sum + p.amount,
    0
  );
    ...
    <main className="min-h-screen bg-gray-50 p-6">
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        {pendingCount > 0 && (
          <div className="flex items-center gap-3 bg-yellow-50 border border-yellow-200 rounded-xl px-5 py-4">
            ...
            <p className="text-sm text-yellow-800 font-medium">
              {pendingCount} payment{pendingCount > 1 ? "s" : ""} waiting for
              your verification
            </p>
          </div>
        )}
        ...
        <section className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <SummaryCard
            label="Verified"
            value={thisMonthVerified.length.toString()}
            sub={`of ${thisMonthPayments.length} submitted`}
            color="green"
          />
          <SummaryCard
          ...
          <SummaryCard
            ...
            value={payments
              .filter(
                (p) =>
                  p.month === currentMonth && p.status === "rejected"
              )
              .length.toString()}
            sub="This month"
            color="red"
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="flex gap-2">
            {(["pending", "all", "verified", "rejected"] as FilterStatus[]).map(
              (s) => (
                <button
                  key={s}
                      ...
                      <td className="px-6 py-4">
                        ...
                        {payment.status === "rejected" &&
                          payment.rejectionReason && (
                            <p className="text-xs text-red-500 mt-1 max-w-[140px]">
                              {payment.rejectionReason}
                            </p>
                          )}
                      </td>
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl max-w-md w-full p-8 space-y-5">
            ...
            <p className="text-sm text-gray-500">
              Provide a reason so the tenant knows what to correct when
              resubmitting.
            </p>
            <textarea
              rows={3}
