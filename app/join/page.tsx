 ...
 * app/join/page.tsx
 *
 * Referral landing page — FIXED version.
 *
 * What changed from previous version:
 * - REMOVED redirect to /register after submission
 * - Referee hits a DEAD END after submitting — shows thank-you screen only
 * - Admin must review in admin/referrals and explicitly invite them
 * - This preserves the admin-controlled invite onboarding flow entirely
 *
 * Flow:
 * 1. Referee opens /join?ref=<tenantDocId>
 * 2. Sees who referred them + what org they're associated with
 * 3. Enters name + email → referral doc created (status: "submitted")
 * 4. Sees thank-you screen — told landlord will be in touch
 * 5. Nothing else happens until admin acts
 */
...
export default function JoinPage() {
  ...
  const validate = async () => {
    ...
    try {
        ...
        /* Referrer must be an active tenant in a live org */
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    /* Check if this email already submitted a referral for this org */
    ...
    try {
      ...
      setStage("done");
      /* ← NO redirect. Dead end. Admin takes it from here. */
    } catch (err) {
      console.error(err);
    ...
    return (
      <CenteredCard>
        ...
        <p className="text-sm text-gray-500 text-center mt-2">
          ...
          Ask the person who sent it to share it again.
        </p>
      </CenteredCard>
    );
  }

  /* ── SUCCESS — dead end, no redirect ──────────────────────────────── */
  if (stage === "done") {
    return (
      <CenteredCard>
    ...
    <CenteredCard>
      ...
      <div className="flex items-center gap-3 bg-indigo-50 border border-indigo-100 rounded-lg px-4 py-3 mb-6">
        ...
        <p className="text-sm text-indigo-800 leading-relaxed">
          <span className="font-semibold">{referrer?.tenantName}</span> thinks
          you wouldd be a great fit for a vacant unit in their building.
        </p>
      </div>

      <h1 className="text-xl font-bold text-gray-900 mb-1">Express your interest</h1>
      <p className="text-sm text-gray-500 mb-5 leading-relaxed">
        Leave your details below. The property manager will review your interest
        and reach out if there is a suitable unit for you. You will not be registered
        until the landlord personally invites you.
      </p>
      ...
      <p className="mt-5 text-xs text-gray-400 text-center">
        Submitting this form does not create an account. The property manager
        will contact you directly if a unit is available.
      </p>
    </CenteredCard>
  );
}
